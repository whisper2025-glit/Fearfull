...
import { useSignIn, useSignUp, useUser } from "@/lib/fake-clerk";
...
const AuthPage = () => {
  ...
  const onVerifyEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err?.errors?.[0]?.longMessage || err?.errors?.[0]?.message || "Verification error";
      toast.error(message);
    } finally {
      setLoading(null);
    }
        ...
        <div className="rounded-lg border bg-card text-card-foreground shadow-sm p-6">
          <div className="space-y-1 text-center mb-6">
            <h1 className="text-2xl font-semibold">
              {mode === "signin" ? "Welcome back" : verificationRequired ? "Verify your email" : "Create account"}
            </h1>
            <p className="text-sm text-muted-foreground">
              {mode === "signin" ? "Sign in to your account" : verificationRequired ? "Enter the 6-digit code we emailed you" : "Sign up for a new account"}
            </p>
          </div>
          ...
          {verificationRequired && (
            <form onSubmit={onVerifyEmail} className="grid gap-4">
              <div className="grid gap-2">
              ...
              <div className="flex items-center gap-2">
                <Button type="submit" disabled={loading === "verify"}>
                  {loading === "verify" && <LoadingSpinner size="sm" className="mr-2" />}
                  Verify email
                </Button>
                <Button
                  type="button"
                  ...
                  onClick={async () => {
                    try {
                      ...
                      toast.error("Couldn't resend code. Try again later.");
                    }
                  }}
                >
                  Resend code
